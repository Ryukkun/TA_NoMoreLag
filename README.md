...
### Packetカウント方式
- 計測方法（パケロス対策）
    - ①・・・パケットが送られてきた時間を追加で記録する。
    - ②・・・常に送られてきたパケットは1tickの間プレイヤー毎にリストに保存する
    1. 計測開始時は ② からプレイヤーの座標と合うパケットを探し、そこからカウントしていく。<br>また、① より開始パケットの時間を取得しておく
    1. 計測中は ② のパケットの数をカウントしていく。
    1. 計測終了時は ② からプレイヤーの座標と合うパケットを探し、そこまでのパケットをカウント。<br>また、①　より終了パケットの時間を取得しておく
- メリット
    - ゲーム退出中はカウントされない
